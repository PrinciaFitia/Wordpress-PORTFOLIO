...
function oblixar_register_components( $components ) {
...
Hooks::prefixed_add_filter( 'components', 'oblixar_register_components', 20 );
Theme::load(
	array(
 ...
 * @return Theme
 */
function oblixar_theme() {
	return Theme::getInstance();
}

function oblixar_assets() {
	return oblixar_theme()->getAssetsManager();
}
...
oblixar_theme()
	...
	->register_menus(
		array(
			'header-menu' => esc_html__( 'Header Menu', 'oblixar' ),
			'footer-menu' => esc_html__( 'Footer Menu', 'oblixar' ),
		)
	);
...
if ( ! apply_filters( 'kubio_is_enabled', false ) ) {
	oblixar_assets()
		...
		->registerStylesheet( 'oblixar-theme', '/theme/theme.css' );
	oblixar_assets()->loadLocalGoogleFonts();
	//	foreach ( Defaults::get( 'fonts', array() ) as $family => $variants ) {
	//		oblixar_assets()->addGoogleFont( $family, $variants );
	//	}

	add_action( 'wp_enqueue_scripts', 'oblixar_print_color_scheme', 0 );
} else {
	oblixar_assets()
		->registerStyle( 'oblixar-theme', oblixar_assets()->getBaseURL() . '/theme/fse-base-style.css' );
	Hooks::prefixed_add_filter( 'skip_google_fonts', '__return_true' );
}

function oblixar_theme_add_woocommerce_support() {
	add_theme_support(
		'woocommerce',
		array(
			'product_grid' => array(
...
add_action( 'after_setup_theme', 'oblixar_theme_add_woocommerce_support' );

add_filter( 'kubio/activation/after_activation_redirect_url', 'oblixar_after_activation_redirect_url' );
function oblixar_after_activation_redirect_url( $url ) {
...
function oblixar_get_builder_plugin_slug() {
...
function oblixar_theme_plugins( $plugins ) {

	$plugin_slug = oblixar_get_builder_plugin_slug();

	$plugins[ $plugin_slug ] = array(
		'name'        => $plugin_slug === 'kubio-pro' ? __( 'Kubio PRO', 'oblixar' ) : __( 'Kubio', 'oblixar' ),
...
Hooks::prefixed_add_filter( 'theme_plugins', 'oblixar_theme_plugins' );
...
Hooks::prefixed_add_action(
	...
	function ( $slug ) {
		...
		if ( $slug === oblixar_get_builder_plugin_slug() ) {
                ...
                <div class="kubio-admin-row get-started-imported notice notice-success">
                    <div>
                        <p class="imported-title">
							...
							echo esc_html(
								sprintf(
									__( '%s design has been successfully imported!', 'oblixar' ),
									oblixar_theme()->getName()
								)
							);
							?>
                        </p>
                        <p class="imported-subtitle">
							...
							echo esc_html(
								sprintf(
									__( '%s design has been successfully imported! You can take a look at your new design or start editing it', 'oblixar' ),
									oblixar_theme()->getName()
								)
							);
							?>
                    ...
                    <div class="button imported-view-site-button">
                        <a href="<?php echo esc_url( site_url() ); ?>">
							<?php echo esc_html( __( 'View site', 'oblixar' ) ); ?>
                        </a>
...
function oblixar_try_online_url() {
	return 'https://kubiobuilder.com/go/try-theme/' . get_template();
}

add_filter( 'kubio/editor-try-online/url', 'oblixar_try_online_url', 10 );

function oblixar_render_header_style() {

	?>
    <base target="_top">
	<?php

	oblixar_theme()->get( 'css' )->render();
}
add_action( 'wp_head', 'oblixar_render_header_style', 100 );
