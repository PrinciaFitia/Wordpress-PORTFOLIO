...
/*!**************************************!*\
  ...
  \**************************************/
(function ($) {
  var $notice_container = $(".oblixar-admin-big-notice--container");
  var selectedFrontPage = 0;

  if (!window.oblixar_admin) {
    return;
  }

  var _window$oblixar_admin = window.oblixar_admin,
      builderStatusData = _window$oblixar_admin.builderStatusData,
      getStartedData = _window$oblixar_admin.getStartedData;
  ...
  function toggleProcessing(value) {
    $(window).on("beforeunload.oblixar-admin-big-notice", function () {
      return true;
    });

    if (value) {
      $(".oblixar-admin-big-notice").addClass("processing");
      $(".oblixar-admin-big-notice .action-buttons").fadeOut();
    } else {
      $(".oblixar-admin-big-notice").removeClass("processing");
    }
  }
  ...
  function installBuilder(callback) {
    ...
    $.get(builderStatusData.install_url).done(function () {
      ...
      $(window).off("beforeunload.oblixar-admin-big-notice");
    });
  }
  ...
  $notice_root = $notice_container.closest(".oblixar-admin-big-notice");
  $custom_close_button = $notice_root.find(".oblixar-notice-dont-show-container");

  if ($custom_close_button.length) {
    $custom_close_button.on("click", function () {
      disableNotice();
      $notice_container.closest(".oblixar-admin-big-notice").find("button.notice-dismiss").click();
    });
  } else {
  ...
  window.oblixar_admin.showOverlay = showOverlay;
})(jQuery);
/******/ })()
;
